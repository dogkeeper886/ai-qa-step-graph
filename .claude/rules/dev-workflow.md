...
> **Plan stage is live (STORY-008).** `dw-plan` produces the plan issue (#85) and
> `dw-tasks` breaks it into tasks (#86). Remaining polish: `dw-story` hand-off (#88),
> `dw-implement` reading the plan (#89), `dw-review-tasks` link check (#90).
...
```
   ...
   dw-create-pr ───► [human review + /review]   open the PR
            │
            ▼
   dw-merge          green `make ci` + human review → merge
```

## The plan issue (STORY-008)
...
| `dw-merge`     | *(is the terminal gate)* | green `make ci` + human review |
...
- **CI** is STORY-002's `make ci` (`up → suite → drift`) + human review, not a new gate.
